...
export const fetchVotes = createAsyncThunk(
    'vote/fetchVotes',
    async () => {
        const response = await fetch('https://radiant-academy-vert.vercel.app/api/vote')
            .then(res => res.json())
...
export const dropVote = createAsyncThunk(
    ...
    async (vote) => {
        try {
            const response = await axios.put(`https://radiant-academy-vert.vercel.app/api/vote/${vote._id}`);
            return response.data
        } catch (error) {
            console.log(error);
        }
